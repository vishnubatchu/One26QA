({
	ApproveSelectedHelper: function(component, event, ApproveRecordsIds) {
          ...
          action.setParams({
           "appRecordId": ApproveRecordsIds,"ObjId": component.get("v.QuoteId")
          });
  ...
  $A.enqueueAction(action);
     ...
     ReassignSelectedHelper: function(component, event, ReassignRecordsIds) {
          ...
          // pass the all selected record's Id's to apex method 
          action.setParams({
              "reassignRecordId": ReassignRecordsIds,"ReasignEmail":component.get("v.eMail"),"ObjId": component.get("v.QuoteId")
          });
          action.setCallback(this, function(response) {
           //store state of response
           var state = response.getState();
           if (state === "SUCCESS") {
            ...
            if (response.getReturnValue() != '') {
             // if getting any error while delete the records , then display a alert msg/
             component.set("v.OperationResult", response.getReturnValue());
